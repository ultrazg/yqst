...
class BlueInvoiceDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: {
        ...
        return (
            <ViewContent
                crumb={[
                    {name: '税票中心'},
                    {name: "发票管理"},
                    {name: "蓝字发票列表", link: '/Pages/BlueInvoiceList'},
                    {name: "蓝字发票详情"}
                ]}
                topBtn = {
                    <div>
                        ...
                        >删除</Button>*/}
                        <Link to={'/Pages/BlueInvoiceList'} style={{marginLeft: 15}}>
    ...
    getInfo() {
        Model.InvoiceBGet({blueId: this.id}, (res) => {
    ...
    getUserLogList() {
        Model.InvoiceMULog({invoiceId: this.id, invoiceManageType: 3}, (res) => {
    ...
    getAdminLogList() {
        Model.InvoiceMALog({invoiceId: this.id, invoiceManageType: 2}, (res) => {
        ...
        let allData = [
            {
                ...
                data: [
                    ...
                    {key: 'invoiceAmount', type: 'Texts', label: '发票金额', span: 12,
                    ...
                    {key: 'redDashedStatus', type: 'Texts', label: '冲红状态', span: 12,
                        value: SwitchStatus.redDashedStatus(data.redDashedStatus)},
            ...
            },
            {
                ...
                data: [
                    ...
                    {key: 'invoiceNumber', type: 'Texts', label: '发票号码', span: 12, value: getVerifyVO.invoiceNumber},
                    {key: 'invoiceTime', type: 'Texts', label: '开票日期', span: 12,
                        value:  data.invoiceTime ? moment(data.invoiceTime).format('YYYY-MM-DD') : ''},
                    {key: 'verifyNumber', type: 'Texts', label: '校验码', span: 12, value: getVerifyVO.verifyNumber},
                    {key: 'invoiceMoney', type: 'Texts', label: '合计金额(不含税)', span: 12,
                        value: NumberFormat.thousandBit(data.invoiceMoney || 0, 2, true)},
                    {key: 'taxPaid', type: 'Texts', label: '税额', span: 12,
                        value: NumberFormat.thousandBit(getVerifyVO.taxPaid || 0, 2, true)},
...
export default BlueInvoiceDetail
