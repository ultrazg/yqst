 ...
 * @LastEditTime : 2021-06-01 11:32:39
 */
...
function SPUSelector({value = {}, onChange, placeholder}) {
        ...
        >
            {
                categorylist && categorylist.map(
                    (item) => {
                        return (
                            <Option data={item} value={item.leaseGoodsParentSn} key={item.leaseGoodsParentSn}
                                    label={item.goodsName}>
                                <span style={{verticalAlign: "center"}}>SPU编号：{item.leaseGoodsParentSn}</span>
                                <br/>
...
SPUSelector.defaultProps = {
    placeholder: "请选择",
};

export default memo(SPUSelector);
