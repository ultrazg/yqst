...
class CancellationApplicationDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: {},
            ...
            <Form ref={this.formRef}>
                <ViewContent
                    crumb={[
                        {name: '支付中心'},
                        {name: "商户账户管理"},
                        {name: "注销申请列表", link: '/Pages/CancellationApplicationList'},
                        {name: "注销申请详情"}
                    ]}
                    topBtn = {
                        <div>
                            {/*<Button type="primary" icon={'edit'}>编辑</Button>*/}
                            <Link to={'/Pages/CancellationApplicationList'} style={{marginLeft: 15}}>
    ...
    getInfo() {
        Model.walletPACGet({cancelId: this.id}, (res) => {
    ...
    // 用户操作记录
    getLogList() {
        Model.walletPALList({id: this.id, type: 3}, (res) => {
    ...
    // 管理者操作记录
    getAdminLogList() {
        Model.walletPAALList({id: this.id, type: 3}, (res) => {
        ...
        let allData = [
            {
                ...
                data: [
                    ...
                    {key: 'cancelSn', type: 'Texts', label: '账户注销申请单号', span: 12, value: data.cancelSn},
                    {key: 'accountId', type: 'Texts', label: '商户账户ID', span: 12, value: data.accountId},
                    {key: 'memo', type: 'Texts', label: '原因', span: 12, value: data.memo},
...
export default CancellationApplicationDetail
