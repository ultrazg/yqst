...
import './MaterialClassLess.less'
import SWTable from 'SWViews/table';
import TabsViewContent from "../../../../baseview/tabsViewContent/TabsViewContent";

class MaterialClassList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            current: 1,
    ...
    getPage = (current = this.state.current, pageSize = this.state.pageSize, keyWord = this.state.keyWord) => {
        MaterialCenterModel.CategoryList({
            keyWord
        }, res => {
    ...
    del = (id) => {
        MaterialCenterModel.CategoryDel({sn: id}, res => {
    ...
    enable = (id, status) => {
        MaterialCenterModel.CategoryEnable({sn: id, categoryStatus: status}, res => {
    ...
    renderTable = () => {
        ...
        const columns = [
            {title: '物资类目名称', dataIndex: 'categoryName'},
            ...
            <>
                ...
                >
                    <a style={record.categoryStatus == 1 ? {color: '#F12C20'} : {}}>
                        {/*categoryStatus	int	物资类目状态 1:启用 2:停用*/}
    ...
    saveOrUpdate = () => {
        ...
        MaterialCenterModel.CategorySave({
    ...
    renderModal = () => {
        ...
        return (
            <Modal
                ...
                title={parentSn ? '添加物资子类目' : sn ? '修改物资类目' : '添加物资类目'}
                className={'sw-modal'}
                destroyOnClose={true}
