...
class InvoiceElementList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '税票中心'}, {name: "基本设置"}, {name: "发票要素列表"}]}
                // tabs={this.tabsConfig()}
                topBtn={
                    <Link to={'/Pages/InvoiceElementEditor'}>
    ...
    getList() {
        Model.InvoiceEPage(this.state.requestPar, (res) => {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/InvoiceElementDetail?id=${res.elementId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default InvoiceElementList
