...
class RedInvoiceList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '税票中心'}, {name: "发票管理"}, {name: "红冲发票列表"}]}
    ...
    getList() {
        Model.InvoiceRedPage(this.state.requestPar, (res) => {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/RedInvoiceDetail?id=${res.redId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
