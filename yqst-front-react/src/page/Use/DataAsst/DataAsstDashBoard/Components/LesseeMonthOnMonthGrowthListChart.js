...
/*
 * @Description  : 月度承租物资增量环比
 * @version      : 0.1
 * @Author       : Chenglin Huang
 * @Date         : 2021-05-21 10:24:00
 * @LastEditTime : 2021-06-01 12:32:44
 */
...
const BASEOPTIONS = baseOptions.LesseeMonthOnMonthGrowthListChart;

export default memo(function LesseeMonthOnMonthGrowthListChart({height, data}) {
  const [lesseeMonthOnMonthGrowthList, setLesseeMonthOnMonthGrowthList] = useState(null);
  const [isModalVisible, setIsModalVisible] = useState(false);
  const statusText = useRef("默认");
  const lesseeMonthOnMonthGrowthListChartIns = useRef();

  const requestLesseeMonthOnMonthGrowthList = useCallback((kindId, {categorySn, leaseGoodsSn, leaseGoodsParentSn}) => {
    request(
      apiInterfaces.lesseeMonthOnMonthGrowthList,
      {
        kindId,
        categorySn,
        leaseGoodsParentSn,
      ...
      (res) => {
        setLesseeMonthOnMonthGrowthList(res.data);
      },
      (err) => {
        console.warn(err);
      },
  ...
  const _onReset = useCallback(() => {
    ...
    setLesseeMonthOnMonthGrowthList(null);
    // setIsModalVisible(false);
  }, []);
  ...
  const _onConfirm = useCallback((values) => {
    switch (values.kind.value) {
      case 1:
        ...
        requestLesseeMonthOnMonthGrowthList(1, {categorySn: values.category.value});
        break;
      case 2:
        // SPU
        statusText.current = values.kind.label + "-" + values.SPU.label + "-" + values.SPU.value;
        requestLesseeMonthOnMonthGrowthList(2, {leaseGoodsParentSn: values.SPU.value});
        break;
      case 3:
        // SKU
        statusText.current = values.kind.label + "-" + values.SKU.label + "-" + values.SKU.value;
        requestLesseeMonthOnMonthGrowthList(3, {leaseGoodsSn: values.SKU.value});
        break;
      default:
        break;
    }
    setIsModalVisible(false);
  }, [requestLesseeMonthOnMonthGrowthList]);
  ...
  const _onLegendChange = useCallback(
    (ev) => {
      ...
      const geoms = lesseeMonthOnMonthGrowthListChartIns.current.geometries;
  ...
  return (
    <>
      <div className={styles.indexTitle}>
        <h2 className={styles.indexH2}>月度承租物资增量环比</h2>
        <a
      ...
      {/*<Chart*/}
      {/*  data={transform(lesseeMonthOnMonthGrowthList || data)}*/}
      {/*  padding="auto"*/}
      {/*  autoFit*/}
      {/*  height={height}*/}
      {/*  onGetG2Instance={*/}
      {/*    (chartIns) => {*/}
      {/*      lesseeMonthOnMonthGrowthListChartIns.current = chartIns;*/}
      {/*    }*/}
      {/*  }*/}
      {/*  scale={{*/}
      {/*    "环比增长率": {*/}
      ...
      {/*  <Axis name="承租增长量" title={{position: "end"}}/>*/}
      ...
      <Charts
        height={height}
        options={cloneDeep(transform(lesseeMonthOnMonthGrowthList || data))}
      />
      <ChartChangeModal
