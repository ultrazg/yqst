...
class AccountApplicationList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '支付中心'}, {name: "商户账户管理"}, {name: "账户申请列表"}]}
    ...
    getList() {
        Model.walletPAAPage(this.state.requestPar, (res) => {
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查收申请单号、企业名称、商户ID', label: '关键字', maxLength: 30},
            {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/AccountApplicationDetail?id=${res.applyId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default AccountApplicationList
