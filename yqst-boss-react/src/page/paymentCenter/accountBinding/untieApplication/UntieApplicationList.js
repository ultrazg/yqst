...
class UntieApplicationList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '支付中心'}, {name: "商户账户绑定"}, {name: "解绑申请列表"}]}
    ...
    getList() {
        Model.walletPSBCPage(this.state.requestPar, (res) => {
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查询解绑申请单、商户账户ID、企业名称', label: '关键字', maxLength: 30},
            {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/UntieApplicationDetail?id=${res.cancelId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default UntieApplicationList
