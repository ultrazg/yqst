...
import {Button, Input, Table, Modal, TreeSelect, message, Form} from 'antd';
...
class FinancialCategory extends Component {
    ...
    showModal(type) {
        this.setState({
            title: type === 'del' ? '删除方案类型' : type === 'edit' ? '修改方案类型' : '添加方案类型',
            modalType: type,
            visible: true,
        });
    }
    ...
    publicClassifyOpera() {
        return <Modal
            ...
            <div style={{marginTop: 10, display: 'flex', flexDirection: 'row', alignItems: 'center'}}>
                <span style={{width: 100, fontSize: 14,}}>方案类型名称：</span>
                <Form.Item
                    rules={[{required: true, message: '请输入方案类型名称'}]}
    ...
    delClassifyOpera() {
        return (<Modal
            ...
            <p style={{color: '#333', fontSize: 16}}>{"是否删除该方案类型"}</p>
        </Modal>);
    }
}

const FinancialCategoryPage = FinancialCategory;
...
export default connect(mapStateToProps)(FinancialCategoryPage)