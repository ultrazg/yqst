...
import SaProductMenu from './SaProductMenu';
import SaProductHomeRouter from './SaProductHomeRouter.js';
import {LeftOutlined} from '@ant-design/icons'

const {SubMenu} = Menu;

class SaProductHome extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    makeLeft() {
        ...
        const menuView = (list = []) => {
            return list.map((item, idx) => {
                ...
                >
                    {
                        item.logo ? <img src={selectedKeys === item.key ? item.hoverLogo : item.logo} alt=""
                                         style={{
                                             width: '22px',
                                             height: '22px',
                                             marginRight: '8px'
            ...
            >
                ...
                >
                    <Button
                        ...
                        icon={<LeftOutlined/>}
                        onClick={() => {
                            this.props.history.push('/pages/appCenter/sellAssistant');
                        }}
                    >返回</Button>
                </div>
                <Menu
