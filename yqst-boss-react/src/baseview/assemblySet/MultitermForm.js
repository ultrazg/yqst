...
class EditableTable extends React.Component {
    ...
    cancel = (current) => {
        ...
        this.setState({editingKey: '', value: value});
        this.props.form.setFieldsValue({optionName: ''});
    };
    ...
    save(form, key) {
        form.validateFields().then(row => {
            ...
            if (index > -1) {
                ...
                form.setFieldsValue({optionName: ''});
            } else {
    ...
    edit(key) {
        this.setState({editingKey: key}, () => {
            let item = this.state.value[key];
            if (item) {
                this.props.form.setFieldsValue({optionName: item.optionName || ''});
            }
        });
    }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if (nextProps.value && isArray(nextProps.value)) {
            if (nextProps.value[0] && nextProps.value[0].key === undefined) {
                ...
                returnObj = {
                    ...returnObj,
                    value: nextProps.value.map((item) => {
                        return {...item}
                    })
                }
            }
        ...
        if (nextProps.edit) {
            returnObj = {
                ...returnObj,
                editingKey: nextProps.value.length - 1
            }
        }
    ...
    addPartners = () => {
        ...
        let contactVOList = this.arrHeadPush(this.state.value, newItem, 0)
        contactVOList.forEach((n, index) => {
            n.index = contactVOList.length - index
        });
        this.props.onChange(contactVOList);
        setTimeout(() => {
            this.setState({
                // value: contactVOList,
                editingKey: contactVOList.length - 1
            });
        }, 1);
    }
