...
class MerchantAccountList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '支付中心'}, {name: "商户账户管理"}, {name: "商户账户列表"}]}
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查收商户账户ID、企业名称、商户ID', label: '关键字', maxLength: 30},
            {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <span style={{color: SwitchNames.accountStatusName(res, 'color')}}>
                        {SwitchNames.accountStatusName(res)}
                    </span>
                }
            },
            {
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/MerchantAccountDetail?id=${res.accountId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
