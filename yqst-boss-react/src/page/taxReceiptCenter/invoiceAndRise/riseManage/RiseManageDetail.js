...
class RiseManageDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: {},
        ...
        return (
            <ViewContent
                crumb={[
                    {name: '税票中心'},
                    {name: "开票信息与抬头管理"},
                    {name: "抬头管理列表", link: '/Pages/RiseManageList'},
                    {name: "抬头管理详情"}
                ]}
                topBtn = {
                    <div>
                        ...
                        >删除</Button>*/}
                        <Link to={'/Pages/RiseManageList'} style={{marginLeft: 15}}>
    ...
    getInfo() {
        Model.InvoiceRGet({riseId: this.id}, (res) => {
    ...
    getLogData() {
        Model.InvoiceILog({invoiceId: this.id, invoiceType: 2}, (res) => {
        ...
        let allData = [
            {
                ...
                key: 'JBKey',
                data: [
                    {key: 'riseId', type: 'Texts', label: '抬头ID', span: 12, value: data.riseId},
                    {key: 'createTime', type: 'Texts', label: '产生时间', span: 12,
                        value: data.createTime ? moment(data.createTime).format('YYYY-MM-DD HH:mm:ss') : ''},
                    {key: 'riseType', type: 'Texts', label: '抬头类型', span: 12,
                        value: SwitchStatus.riseType(data.riseType)},
                    {key: 'riseStatus', type: 'Texts', label: '抬头状态', span: 12,
                        value: SwitchStatus.riseStatus(data.riseStatus)},
                    ...
                    {key: 'personId', type: 'Texts', label: '个人用户ID', span: 12, value: data.personId},
                    {key: 'personName', type: 'Texts', label: '个人用户名称', span: 12, value: data.personName},
                ],
                style: {},
            },
            {
                ...
                key: 'TTKey',
                data: [
                    {key: 'riseName', type: 'Texts', label: '抬头名称', span: 12, value: data.riseName},
...
export default RiseManageDetail
