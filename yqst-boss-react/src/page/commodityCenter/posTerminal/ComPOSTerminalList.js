...
class ComPOSTerminalList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    getList() {
        Model.ShopPage(this.state.requestPar, (res) => {
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查询销售终端名称', label: '关键字', maxLength: 30},
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return res ? moment(res).format('YYYY-MM-DD') : '';
                }
            },
            {
                title: '终端创建时间',
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/ComPOSTerminalDetail?id=${res.shopId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default ComPOSTerminalList
