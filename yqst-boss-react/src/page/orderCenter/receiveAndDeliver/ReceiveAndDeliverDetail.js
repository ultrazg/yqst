...
class ReceiveAndDeliverDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: {
            ...
            <Form ref={this.formRef}>
                <ViewContent
                    crumb={[
                        {name: '订单中心'},
                        {name: "货单管理"},
                        {name: "收发货单管理列表", link: '/Pages/ReceiveAndDeliverList'},
                        {name: "收发货单管理详情"}
                    ]}
                    topBtn = {
                        <div>
                            ...
                            <Link to={'/Pages/ReceiveAndDeliverList'} style={{marginLeft: 15}}>
    ...
    getInfo() {
        Model.OrderDGet({deliveryId: this.id}, (res) => {
    ...
    getLogData() {
        Model.OrderWLList({deliveryId: this.id, deliveryType: 1}, (res) => {
    ...
    makeBaseView() {
        ...
        let {buyerVO, sellerVO} = data;
        let {usersLogVOList, adminLogVOList} = logData;

        let noSetDataOne = [
            {key: 'deliverySn', type: 'Texts', label: '收发货单编号', span: 12, value: data.deliverySn},
            {key: 'status', type: 'Texts', label: '状态', span: 12, value: SwitchNames.deliveryStatusName(data.status)},
            {key: 'orderSn', type: 'Texts', label: '订单编号', span: 12, value: data.orderSn},
            {key: 'alreadyPay', type: 'Texts', label: '商品总价(含税)', span: 12,
                value: NumberFormat.thousandBit(data.alreadyPay || 0, 2, true)},
            {key: 'saleSn', type: 'Texts', label: '交易单号', span: 12, value: data.saleSn},
        ...
        return <div>
            ...
            >
                ...
                <Row style={{
                    ...
                    <Col span={8}>发货总金额（含税）：{NumberFormat.thousandBit(data.deliveryTotal, 2, true)} </Col>
                    <Col span={8}>已发货款合计（含税）：{NumberFormat.thousandBit(data.itemTotal, 2, true)}</Col>
                    <Col span={8}>未发货款合计（含税）：{NumberFormat.thousandBit(data.bettyTotal, 2, true)}</Col>
                </Row>
            </Card>
    ...
    makeSHView(){
        let {data} = this.state;
        let {afterCaseListVO} = data;

        return <Card
            type="inner"
            title={
                <div>
                    <span style={{marginRight: 15}}>货单售后信息</span>
                    ( 货单售后状态:<span style={{fontWeight: 600, marginLeft: 5}}>
                        {SwitchNames.afterStatusName(data.afterStatus)}
                    </span> )
                </div>
            }
        >
            <Timeline>
                {
                    afterCaseListVO && afterCaseListVO.map((item, idx) => {
                        return <Timeline.Item key={'ac_' + idx}>
                            <div>
                                <span>售后单号：{item.afterSaleSn}</span>
                                <span style={{marginLeft: 35}}>产生时间：{item.createTime ? moment(item.createTime).format('YYYY-MM-DD HH:mm:ss') : ''}</span>
                            </div>
                        </Timeline.Item>
                    })
                }
                {
                    afterCaseListVO && afterCaseListVO.length <= 0 && <div style={{textAlign: 'center', color: '#ccc', fontSize: 20}}>
                        暂无售后信息...
                    </div>
                }
            </Timeline>
...
export default ReceiveAndDeliverDetail
