...
class CloudServeRuleList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '云服务中心'}, {name: "云服务及规则管理"}, {name: "云服务规则列表"}]}
                // tabs={this.tabsConfig()}
                topBtn={
                    <Link to={'/Pages/CloudServeRuleEdit'}>
    ...
    getList() {
        Model.CServeSGPage(this.state.requestPar, (res) => {
            this.setState({
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查询ID、编码、名称、所属云服务', label: '关键字', maxLength: 30},
            // {
            //     key: 'a', type: 'Select', value: '', placeholder: '请选择可见性', label: '可见性',
            //     list: [
            //         {value: '', name: '全部'},
            //         {value: 1, name: '全部可见'},
            //         {value: 2, name: '白名单可见'},
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/CloudServeRuleDetail?id=${res.id}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default CloudServeRuleList
