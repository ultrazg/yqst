...
class BindingApplicationList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '支付中心'}, {name: "商户账户绑定"}, {name: "绑定申请列表"}]}
    ...
    getList() {
        Model.walletPSBAPage(this.state.requestPar, (res) => {
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查注绑定申请单、商户账户ID、企业名称', label: '关键字', maxLength: 30},
            {
                key: 'applyStatus', type: 'Select', value: '', placeholder: '请选择申请状态', label: '申请状态',
                list: [
                    {value: '', name: '全部'},
                    {value: 1, name: '待处理'},
                    {value: 2, name: '已通过'},
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <span style={{color: SwitchNames.applyStatusName(res, 'color')}}>
                        {SwitchNames.applyStatusName(res)}
                    </span>
                }
            },
            {
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/BindingApplicationDetail?id=${res.bindId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default BindingApplicationList
