...
class ReceiveAndDeliverList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '订单中心'}, {name: "货单管理"}, {name: "收发货单管理列表"}]}
    ...
    getList() {
        Model.OrderDPage(this.state.requestPar, (res) => {
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查询单号、销售方、购买方', label: '关键字', maxLength: 30},
            {
                key: 'deliveryStatus', type: 'Select', value: '', placeholder: '请选择收发货状态', label: '收发货状态',
                list: [
                    {value: '', name: '全部'},
                    {value: 1, name: '待发货'},
                    {value: 2, name: '已发货'},
            ...
            {
                key: 'afterStatus', type: 'Select', value: '', placeholder: '请选择售后状态', label: '售后状态',
                list: [
                    {value: '', name: '全部'},
                    {value: 0, name: '未发起'},
                    {value: 1, name: '已发起'},
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return SwitchNames.deliveryStatusName(res);
                }
            },
            {
                title: '售后状态',
                ...
                render: (res) => {
                    return SwitchNames.afterStatusName(res);
                }
            },
            {
                title: '商品总价(含税)',
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/ReceiveAndDeliverDetail?id=${res.deliveryId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default ReceiveAndDeliverList
