...
class ProductApplyForDetail extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    getDetail = () => {
        ...
        if (id) {
            ...
            ProductCentreModel.ApplyDetail({
                applySn: id
            }, res => {
    ...
    render() {
        ...
        return (
            <TabsViewContent
                ...
                topBtn={
                    this.state.isLoaded && this.state.submitData.applyStatus == 1 ? <>
                        <Popconfirm
                            title="确认并创建平台产品?"
                            onConfirm={() => {
                                ProductCentreModel.ApplyAgree({
                                    applySn: this.state.submitData.applySn
                                }, (res) => {
                                    message.info("操作成功");
                                    this.getDetail();
                                }, () => {
            ...
            >
                ...
                {this.state.rejectVisi ? <RejectModal
                    ok={(memo) => {
                        ProductCentreModel.ApplyReject({
                            applySn: this.state.submitData.applySn,
                            rejectReason: memo
                        }, () => {
                            this.setState({rejectVisi: false})
                            this.getDetail()
                        }, () => {
                        })
                    }}
...
class RejectModal extends Component {
    ...
    render() {
        ...
        return <Modal
            ...
            onOk={() => {
                if (CheckInput.isNull(this.state.memo)) {
                    return message.info("驳回原因不能为空")
                }
                ok && ok(this.state.memo);
            }}>
            <Input.TextArea
...
export default ProductApplyForDetail;
