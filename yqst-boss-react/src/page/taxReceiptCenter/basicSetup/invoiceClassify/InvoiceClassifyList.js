...
class InvoiceClassifyList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '税票中心'}, {name: "基本设置"}, {name: "发票分类管理"}]}
                // tabs={this.tabsConfig()}
                topBtn={
                    <Link to={'/Pages/InvoiceClassifyEditor'}>
    ...
    getList() {
        Model.InvoiceCPage(this.state.requestPar, (res) => {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/InvoiceClassifyDetail?id=${res.classifyId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default InvoiceClassifyList
