...
class APChannelList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '支付中心'}, {name: "基础设置"}, {name: "收支付渠道列表"}]}
                // tabs={this.tabsConfig()}
                topBtn={
                    <Link to={'/Pages/APChannelEdit'}>
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查询渠道ID、渠道编码、渠道名称', label: '关键字', maxLength: 30},
            {
                key: 'payType', type: 'Select', value: '', placeholder: '请选择收支付类型', label: '收支付类型',
                list: [
                    {value: '', name: '全部'},
                    {value: 1, name: '线上'},
                    {value: 2, name: '线下'},
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/APChannelDetail?id=${res.channelId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
