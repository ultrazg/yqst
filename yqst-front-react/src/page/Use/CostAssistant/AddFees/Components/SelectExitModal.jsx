...
class SelectEntryModal extends Component {
    constructor(props) {
        ...
        this.columns = [
                ...
                render: (res, record) => {
                    return <>
                        <Button type={'link'} onClick={() => {
                            this.setState({
                                leaseExitSn: record.leaseExitSn,
            ...
            },
            {
                title: '退场单号',
                render: record => {
                    return <span>{record.leaseExitSn}</span>
                }
            },
            {
                title: '退场时间',
                render: record => <span>{record.exitTime ? record.exitTime : ''}</span>
            },
            {
                title: '车牌号',
                ...
                </Modal>
                {
                    this.state.exitDetailModalVisi
                        ? <ExitDetailModal
                            ...
                            leaseExitSn={this.state.leaseExitSn}
                            onClose={() => {
                                this.setState({
                                    exitDetailModalVisi: false
                                });
    ...
    getList = () => {
        CostAssistantModel.LessorExitPage(this.state.requestPar, res => {
            this.setState({
