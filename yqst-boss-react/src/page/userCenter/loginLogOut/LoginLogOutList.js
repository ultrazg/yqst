...
class LoginLogOutList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '用户中心'}, {name: "用户日志管理"}, {name: "登录登出日志"}]}
    ...
    getList() {
        Model.UserLLPage(this.state.requestPar, (res) => {
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查询序号、用户ID、企业ID', label: '关键字', maxLength: 30},
            ...
            {
                key: 'type', type: 'Select', value: '', placeholder: '请选择类型', label: '类型',
                list: [
                    {value: '', name: '全部'},
                    {value: 1, name: '登录'},
                    {value: 2, name: '登出（主动）'},
...
export default LoginLogOutList
