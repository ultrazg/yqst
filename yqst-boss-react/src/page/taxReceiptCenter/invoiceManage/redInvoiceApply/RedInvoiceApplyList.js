...
class RedInvoiceApplyList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '税票中心'}, {name: "发票管理"}, {name: "红冲发票申请列表"}]}
    ...
    getList() {
        Model.InvoiceRAPage(this.state.requestPar, (res) => {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/RedInvoiceApplyDetail?id=${res.redApplyId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default RedInvoiceApplyList
