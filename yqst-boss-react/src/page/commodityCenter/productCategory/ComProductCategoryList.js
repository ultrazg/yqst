...
class ComProductCategoryList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '商品中心'}, {name: "产品管理"}, {name: "产品类目列表"}]}
    ...
    getList() {
        Model.SellerGCPage(this.state.requestPar, (res) => {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/ComProductCategoryDetail?id=${res.categoryId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default ComProductCategoryList
