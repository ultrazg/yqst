...
class RefundManageList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            list: [],
    ...
    render() {
        return (
            <TabsViewContent
                crumb={[{name: '支付中心'}, {name: "用户资金管理"}, {name: "退款管理列表"}]}
    ...
    getList() {
        Model.walletPRefundPage(this.state.requestPar, (res) => {
    ...
    makeHeadSearch() {
        const searchDatas = [
            {key: 'keyWord', type: 'Input', value: '', placeholder: '可查收退款单号、付款方、收款方', label: '关键字', maxLength: 30},
            {
    ...
    makeTable() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (res) => {
                    return SwitchName.refundTypeName(res);
                }
            },
            {
                title: '状态',
            ...
            },
            {
                ...
                render: (res) => {
                    return <div>
                        <Link to={`/Pages/RefundManageDetail?id=${res.refundId}`}>
                            查看
                        </Link>
                        {/*<span style={{
                            margin: '0 10px',
...
export default RefundManageList
